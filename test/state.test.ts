...
import { reduce, IDLE, WORKING, type StateSnapshot } from "../lib/state.ts";
...
test("session_start while busy restores WORKING and keeps prior tool", () => {
  ...
  assert.deepEqual(reduce(prev, { type: "session_start", isIdle: false }), { state: "working", tool: "bash" } as StateSnapshot);
});
...
test("tool_execution_end clears the tool name but stays working", () => {
  const prev: StateSnapshot = { state: "working", tool: "bash" };
  assert.deepEqual(reduce(prev, { type: "tool_execution_end" }), { state: "working", tool: "" } as StateSnapshot);
});

test("agent_settled idle → IDLE", () => {
  assert.deepEqual(reduce({ state: "working", tool: "bash" } as StateSnapshot, { type: "agent_settled", isIdle: true }), IDLE);
});
