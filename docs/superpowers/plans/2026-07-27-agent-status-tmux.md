...
- [ ] **No `set -g` audit**: `grep -rn 'set -g\|set-option -g\|set-option  *-g' lib/ extensions/` must return **zero** matches — the extension never writes a global option. In `tmux/agent-status.tmux`, the only allowed `set -g` lines are `set -g @agent_status_format` and `set -g @agent_window_tab` (defining new user options); verify with `grep -n 'set -g' tmux/agent-status.tmux` and confirm every match is one of those two keys.
